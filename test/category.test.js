...
describe("GET /api/categories", () => {
    ...
    it('should return 400 if limit is not a number', async () => {
        const response = await request(app).get('/api/categories').query({ limit: 'abc' });
    ...
    it("should return 400 if page count is not a number", async () => {
        const response = await request(app).get('/api/categories').query({ page_count: "cde"});
        expect(response.statusCode).toBe(400);
        expect(response.body).toEqual({ success: false, message: "bad request" });
    });

    it("should return 401 if limit is greater than 20", async () => {
        const response = await request(app).get('/api/categories').query({ limit: 24 });
        expect(response.statusCode).toBe(401);
        expect(response.body).toEqual({ success: false, message: "you can only request 20 records per page" });
    })

    it("should return categories with default pagination values", async () => {
        const mockCategories = [{ category: "Fiction" }, {category: "Non-Fiction" }];

        prisma.category.findMany = jest.fn().mockResolvedValue(mockCategories)

        const response = await request(app).get('/api/categories');
        expect(response.statusCode).toBe(200);
        expect(response.body).toEqual({ success: true, message: ["Fiction", "Non-Fiction"]});
    ...
    it("should 200 with a number of page and number of limit", async () => {
        const mockCategories = [{ category: "Adventure"}, { category: "Science" }, { category: "Literature" }];

        prisma.category.findMany = jest.fn().mockResolvedValue(mockCategories);

        const response = await request(app).get('/api/categories').query({ page_count: 0, limit: 3});
        expect(response.statusCode).toBe(200);
        expect(response.body).toEqual({ success: true, message: ["Adventure", "Science", "Literature"] });
        expect(prisma.category.findMany).toHaveBeenCalledWith({
            skip: 0,
            take: 3
        })
